...
main(void)
{
	...
	window = glfwCreateWindow(1024, 768, "Tutorial 06 - Keyboard and Mouse", NULL, NULL);
	if (window == NULL)
	{
		fprintf(
			stderr,
	...
	do
	{
		...
		glVertexAttribPointer(
			0,		  // attribute. No particular reason for 0, but must
		...
		glBindBuffer(GL_ARRAY_BUFFER, uvbuffer);
		glVertexAttribPointer(
			1,		  // attribute. No particular reason for 1, but must
					  // match the layout in the shader.
			2,		  // size : U+V => 2
			GL_FLOAT, // type
			GL_FALSE, // normalized?
